...
class aviao{
  constructor(totalAssentos,fileiraEsquerda,fileiraMeio=0,fileiraDireita=0){
...
function iniciaAssentos(aviao){
  ...
  while (count < totalAssentos){
    if(isNull){
      ...
      if((count + 1)%seatsRow === (fesq) || (count + 1)% seatsRow === (fesq + fmeio)){isNull=true;}

      let seatNumber = alfabeto[(Math.floor(count/seatsRow)).toString()].concat((count%seatsRow)+1);
      count++;
...
class AirplaneSeats extends Component {
  ...
  render() {
    ...
    return (
      ...
      <div>
        <h2>Escolha seus {seatChoose} assentos: </h2>
